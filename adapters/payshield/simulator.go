...
func (s *Simulator) dispatch(cmd string, data []byte) (respCode, errCode string, respData []byte) {
	...
	switch cmd {
	case s.cfg.EncryptPINCmd:
		errCode, respData = s.encryptPIN(data)
...
func (s *Simulator) encryptPIN(data []byte) (string, []byte) {
	...
	format, ok := s.cfg.formatFromCode(code)
	if !ok {
		return codeFormatError, nil
	}
	out, err := s.v.EncryptPINBlock(keyRef, format, pin, pan)
	if err != nil {
