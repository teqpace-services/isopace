...
// payment HSM as the full Isopace [vault.Vault]: [vault.PINEncryptor]
// (issuer-context clear-PIN encrypt), [vault.PINTranslator] (PCI-secure PIN
// translate, the operation a stock PKCS#11 HSM cannot do), and [vault.Macer]
// (ISO 9797-1 message MAC). A payment HSM performs every one of these inside the
// device, so the adapter satisfies the whole façade. It lives in a separate
// module so the stdlib-only core gains no HSM-transport dependency.
//
...
type Config struct {
	...
	// Reference Manual. Defaults: EncryptPINCmd "BA", TranslateCmd "CC",
	// GenerateMACCmd "M6", VerifyMACCmd "M8".
	EncryptPINCmd  string
	TranslateCmd   string
...
// Vault is a payShield-backed vault. A payment HSM performs every key-management
// operation inside the device, so this adapter implements the full [vault.Vault]:
// [vault.PINEncryptor] (issuer-context clear-PIN encrypt), [vault.PINTranslator]
// (PCI-secure PIN translate), and [vault.Macer] (ISO 9797-1 MAC).
type Vault struct {
	cfg Config
	cl  *client
}
...
// EncryptPINBlock implements vault.PINEncryptor via the device's clear-PIN
// encrypt command: the device encodes pin for format and enciphers the resulting
// block under the named PIN key, returning the encrypted block. Because it takes
// the clear PIN it is an issuer / trusted-context operation (see
// vault.PINEncryptor) — a switch never holds the clear PIN and uses TranslatePIN.
func (v *Vault) EncryptPINBlock(keyRef string, format vault.PINBlockFormat, pin, pan string) ([]byte, error) {
	fc, ok := v.cfg.formatCode(format)
	if !ok {
		return nil, &HostError{Op: "EncryptPINBlock", Code: codeFormatError}
	}
	var f fieldWriter
	f.str(keyRef)
	f.str(fc)
	...
	ec, data, err := v.cl.do(v.cfg.EncryptPINCmd, f.b)
	if err != nil {
		return nil, err
	}
	if ec != codeOK {
		return nil, &HostError{Op: "EncryptPINBlock", Code: ec}
	}
