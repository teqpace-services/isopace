...
A reference adapter that exposes a **Thales payShield** payment HSM as the **full
`vault.Vault`** — `vault.PINEncryptor` (issuer-context clear-PIN encrypt),
`vault.PINTranslator` (PCI-secure PIN translate), and `vault.Macer` (ISO 9797-1
message MAC) — over the payShield **host-command protocol**. A payment HSM performs
every one of these inside the device, so the adapter satisfies the whole façade.
Separate module; this module is itself stdlib-only (it depends only on the core for
...
`vault.PINTranslator` and `vault.PINEncryptor`. This adapter, backed by a payment
HSM, advertises the **full `vault.Vault`**.
...
// Issuer context (clear PIN present): encrypt a PIN block under a PIN key.
block, err := v.EncryptPINBlock("zpk-issuer", vault.ISO0, "1234", pan)
```
...
|---|---|---|---|
| `EncryptPINBlock` | `BA` | `BB` | PIN-key token, PIN-block format code, clear PIN, PAN → encrypted PIN block |
...
The test suite asserts the adapter's MAC and ISO0 PIN-block encrypt equal the
`vault` software reference, that an ISO0→ISO3→ISO0 translate round-trips, and that
unknown keys / unsupported algorithms surface correctly — all in-process, no
external dependency.

## What's real vs. what's a stand-in

**Real / faithful:** the capability surface (the full `vault.Vault` —
`PINEncryptor` + `PINTranslator` + `Macer`); the host-command framing and
request/response-code convention; key-token references; PIN-block-format and
MAC-algorithm selection; error-code handling.
