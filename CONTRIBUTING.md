# Contributing to Isopace

Thanks for your interest in Isopace, a product of Teqpace Services Ltd.
...
- **External contributors** must agree to the [CLA](CLA.md) before their first
  contribution is merged. The CLA is a broad, irrevocable, sublicensable
  copyright + patent license grant — **you keep copyright in your work**; you do
  not assign it away.
- **Teqpace employees** contribute work made for hire already owned by Teqpace
  and do not need to sign the CLA.

A single contribution made available *only* under the AGPL, without the CLA
grant, would prevent Teqpace from offering the commercial license — so this is
non-negotiable for inbound code.
...
Every source file must begin with this header (adjust the comment syntax per
language; Go shown):
...
Copyright is held by **Teqpace Services Ltd.** (not the individual author).
Add yourself to [AUTHORS](AUTHORS) in the same change.
...
- Target Go 1.26+.
- Keep packages idiomatic; prefer `context.Context`, typed errors, and table-driven tests.
...
- Reference the relevant section of `ARCHITECTURE.md` where applicable.
